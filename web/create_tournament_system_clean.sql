-- Create Tournament System (Clean Version)
...
DROP TABLE IF EXISTS public.tournament_rankings CASCADE;
...
CREATE INDEX idx_tournament_matches_round_id ON public.tournament_matches(round_id);
CREATE INDEX idx_tournament_rankings_tournament_id ON public.tournament_rankings(tournament_id);
...
BEGIN
    ...
    -- For now, create matches with random player assignments
    -- This is a simplified version - in a real implementation, you'd want more sophisticated pairing logic
    
    -- Create matches for each court (up to the number of courts available)
    FOR i IN 1..LEAST(v_courts, FLOOR(v_player_count / 4)) LOOP
        ...
        SELECT 
            ...
            v_round_id, 
            i,
            (SELECT id FROM public.tournament_players WHERE tournament_id = p_tournament_id ORDER BY RANDOM() LIMIT 1 OFFSET (i-1)*4),
            (SELECT id FROM public.tournament_players WHERE tournament_id = p_tournament_id ORDER BY RANDOM() LIMIT 1 OFFSET (i-1)*4 + 1),
            (SELECT id FROM public.tournament_players WHERE tournament_id = p_tournament_id ORDER BY RANDOM() LIMIT 1 OFFSET (i-1)*4 + 2),
            (SELECT id FROM public.tournament_players WHERE tournament_id = p_tournament_id ORDER BY RANDOM() LIMIT 1 OFFSET (i-1)*4 + 3);
