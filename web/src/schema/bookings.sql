...
create table public.bookings (
  ...
  constraint bookings_table_number_check check (table_number > 0),
  constraint bookings_time_check check (end_time > start_time),
  constraint bookings_total_amount_check check (total_amount >= 0),
  constraint bookings_number_of_players_check check (number_of_players between 2 and 8)
...
create index if not exists bookings_venue_table_time_idx on public.bookings 
using btree (venue_id, table_number, start_time, end_time) tablespace pg_default;
...
-- Prevent overlapping bookings for the same table
