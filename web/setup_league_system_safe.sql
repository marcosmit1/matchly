-- Safe League System Setup
-- This script handles existing tables and ensures proper creation order

-- First, let's check what exists and clean up if needed
DO $$
BEGIN
    -- Drop existing tables in reverse dependency order if they exist
    DROP TABLE IF EXISTS league_promotion_history CASCADE;
    DROP TABLE IF EXISTS league_box_standings CASCADE;
    DROP TABLE IF EXISTS league_matches CASCADE;
    DROP TABLE IF EXISTS league_box_assignments CASCADE;
    DROP TABLE IF EXISTS league_boxes CASCADE;
    ...
    DROP FUNCTION IF EXISTS start_league(UUID);
    DROP FUNCTION IF EXISTS start_league_test(UUID);
    DROP FUNCTION IF EXISTS update_leagues_updated_at();
    DROP FUNCTION IF EXISTS update_league_matches_updated_at();
    DROP FUNCTION IF EXISTS update_league_box_standings_updated_at();
    DROP FUNCTION IF EXISTS initialize_player_standings();
END $$;
...
CREATE TABLE league_box_assignments (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    league_id UUID NOT NULL REFERENCES leagues(id) ON DELETE CASCADE,
...
-- 5. Create league_matches table (this is the one that was causing issues)
...
CREATE TABLE league_box_standings (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    league_id UUID NOT NULL REFERENCES leagues(id) ON DELETE CASCADE,
    box_id UUID NOT NULL REFERENCES league_boxes(id) ON DELETE CASCADE,
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
CREATE TABLE league_promotion_history (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    league_id UUID NOT NULL REFERENCES leagues(id) ON DELETE CASCADE,
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
CREATE INDEX idx_league_boxes_league_id ON league_boxes(league_id);
CREATE INDEX idx_league_boxes_level ON league_boxes(level);

CREATE INDEX idx_league_box_assignments_league_id ON league_box_assignments(league_id);
CREATE INDEX idx_league_box_assignments_box_id ON league_box_assignments(box_id);
CREATE INDEX idx_league_box_assignments_user_id ON league_box_assignments(user_id);
...
CREATE INDEX idx_league_box_standings_league_id ON league_box_standings(league_id);
CREATE INDEX idx_league_box_standings_box_id ON league_box_standings(box_id);
CREATE INDEX idx_league_box_standings_user_id ON league_box_standings(user_id);
CREATE INDEX idx_league_box_standings_points ON league_box_standings(points DESC);

CREATE INDEX idx_league_promotion_history_league_id ON league_promotion_history(league_id);
CREATE INDEX idx_league_promotion_history_user_id ON league_promotion_history(user_id);
