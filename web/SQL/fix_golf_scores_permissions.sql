...
-- 1. User is tournament creator
-- 2. User is in the same fourball as the participant
...
ON public.golf_scores FOR INSERT TO authenticated WITH CHECK (
  ...
  OR
  -- Players can insert scores for their fourball
  EXISTS (
    SELECT 1 
    FROM public.golf_tournament_participants AS scorer
    ...
    WHERE scorer.user_id = auth.uid()
      AND scorer.tournament_id = golf_scores.tournament_id
...
ON public.golf_scores FOR UPDATE TO authenticated USING (
  EXISTS (
    SELECT 1 FROM public.golf_tournaments 
    WHERE id = tournament_id 
    AND created_by = auth.uid()
