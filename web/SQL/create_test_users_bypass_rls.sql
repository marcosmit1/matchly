-- Create test users bypassing RLS policies
-- This script creates test users by using the service role to bypass RLS

-- First, let's check the current RLS policies
SELECT schemaname, tablename, policyname, permissive, roles, cmd, qual, with_check
FROM pg_policies 
WHERE tablename IN ('users', 'league_participants')
ORDER BY tablename, policyname;

-- Temporarily disable RLS for users table (only for this session)
...
SELECT COUNT(*) as test_users_created FROM users WHERE email LIKE '%matchly.test%';
